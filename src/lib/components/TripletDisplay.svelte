<script lang="ts">
    ...
    let data = $derived($tripletAnalysis?.triplets || []);
    let totalDraws = $derived($tripletAnalysis?.totalDraws || 1);
...
<div class="space-y-4">
    <div class="flex items-center justify-between">
        <h3 class="text-base sm:text-lg font-bold text-gray-800">
            Most Appearing Triplets ({$selectedLotto?.name})
        </h3>
        <button
            onclick={loadTripletAnalysis}
            disabled={$isAnalyzingTriplets}
        ...
        >
            <svg
                class="w-4 h-4"
                class:animate-spin={$isAnalyzingTriplets}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
            >
            ...
            <span>{$isAnalyzingTriplets ? "Analyzing..." : "Refresh"}</span>
        </button>
    </div>

    {#if $isAnalyzingTriplets && data.length === 0}
                ...
                >
                    <div class="flex flex-col gap-2">
                        <div class="flex gap-1.5">
                            {#each triplet.Numbers as number}
                                <span
                    ...
                    <div class="text-right">
                        ...
                        <div class="text-[10px] text-gray-500 font-medium">
                            {formatPercentage(triplet.Count)}
                        </div>
