...
import { calculateMostAppearingTriplets } from '$lib/server/analysis';
...
export const GET: RequestHandler = async ({ url }) => {
    try {
        ...
        const triplets = calculateMostAppearingTriplets(history, 21);
        ...
        console.error('Triplet Analysis API error:', error);
        return json({
            success: false,
            error: error.message
        }, { status: 500 });
