...
import { calculateMostAppearingPairs } from '$lib/server/analysis';
...
export const GET: RequestHandler = async ({ url }) => {
    try {
        ...
        const pairs = calculateMostAppearingPairs(history, 21);
        ...
        console.error('Pair Analysis API error:', error);
        return json({
            success: false,
            error: error.message
        }, { status: 500 });
